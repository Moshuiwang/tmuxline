...
| `claude/tmux-claude-age.sh` | `~/.claude/` | 状态持续时长、闪烁开关、残留清理 |
| `claude/tmux-sysload.sh` | `~/.claude/` | 会话徽标下面的整机 CPU / 内存占用 `C23% M61%`(宽屏;超 80% 黄、超 90% 红) |
...
| `tests/` | — | `tests/run.sh` 跑全部测试(bash 燃速函数 / 渲染器 / CPU 内存段 / codex 侧 python),不碰真实数据 |
